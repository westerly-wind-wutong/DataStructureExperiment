...
public class SqList {
    ...
    void DelElem_iToj(SqList L, int i, int j) {
        if(i<=L.length&&j<=L.length&&i<=j) {
            for (int k = 0; k <= L.length-j-1; k++){
                L.data[i+k-1]=L.data[j+k];
            }
    ...
    int menu1() {
        ...
        System.out.print("7.删除从第i个位置到第j个位置范围内的元素\n");
        i = sca.nextInt();
        return i;
    }
    ...
    void menu2(SqList L) {//**
        ...
        int i = 0, j = 0, x = 0, chack = 0, result = 0;
        int[] e=new int[1];
        ...
        while (chack != 6) {
            ...
            switch (chack) {
                ...
                case 6:
                    ...
                    return;
                case 7:
                    System.out.println("输入你需要 删除的第i个元素:\n");
                    i = sca.nextInt();
                    System.out.println("输入你需要 删除的第j个元素:\n");
                    j = sca.nextInt();
                    DelElem_iToj(L,i,j);
                    break;
                default:
